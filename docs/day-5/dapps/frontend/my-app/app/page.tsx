...
export default function Page() {
  ...
  // 🔹 BACKEND STATE
  const [backendValue, setBackendValue] = useState<{ value: number; blockNumber: number; updatedAt: string } | null>(null);
  ...
  const fetchBackendData = async () => {
    try {
      ...
      const [valueRes, eventsRes] = await Promise.all([
        fetch(`${baseUrl}/blockchain/value`),
        fetch(`${baseUrl}/blockchain/events`)
      ]);

      if (valueRes.ok) setBackendValue(await valueRes.json());
      if (eventsRes.ok) setBackendEvents(await eventsRes.json());
    } catch (error) {
    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-background text-foreground bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-gray-950 to-black">
      ...
      <div className="w-full max-w-md glass-panel rounded-3xl p-6 lg:p-8 shadow-2xl space-y-6 animate-fade-in border border-white/5">
        ...
        <div className="space-y-4 pt-4 border-t border-white/5">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Backend Data</h2>
            <button onClick={fetchBackendData} className="text-primary hover:text-primary-hover p-1"><RefreshCw className="w-4 h-4" /></button>
          </div>
          <div className="bg-black/20 rounded-xl p-4 border border-white/5 space-y-2 text-sm text-gray-300">
            <div className="flex justify-between"><span>Value:</span> <span className="text-white font-bold">{backendValue?.value ?? '-'}</span></div>
            <div className="flex justify-between"><span>Block:</span> <span>{backendValue?.blockNumber ?? '-'}</span></div>
            <div className="flex justify-between"><span>Last Updated:</span> <span>{backendValue?.updatedAt ? new Date(backendValue.updatedAt).toLocaleString() : '-'}</span></div>
          </div>

          <div className="space-y-2">
            <h3 className="text-xs font-semibold text-gray-500 uppercase">Recent Events</h3>
            <div className="max-h-32 overflow-y-auto space-y-2 custom-scrollbar">
              {backendEvents.map((evt, i) => (
                <div key={i} className="bg-white/5 p-2 rounded text-xs flex justify-between items-center">
                  <span>Value: {evt.value}</span>
                  <a href={`https://testnet.snowtrace.io/tx/${evt.txHash}`} target="_blank" className="text-blue-400 flex items-center gap-1">Tx <ExternalLink className="w-3 h-3" /></a>
                </div>
              ))}
            </div>
          </div>
